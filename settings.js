document.addEventListener("DOMContentLoaded", function() {
   //blur background
    ...
    slide.onclick = function() {
        ...
        document.getElementById('bblur').classList.toggle('blurred');
};
...
//JSON data
  const json_data = {
    ...
    "person3":[
        ...
        {"head":"Amazon"},
        {"text1":"Job Status"},
        {"text2":"We would love to move forward; however, we still nedd your W2 paperwork."}
    ],
    "person4":[
    ...
    "person5":[
        ...
        {"text2":"IMPORTANT: Email Verification Code: 987643; Do not tell anyone this."}
    ],
    "person6":[
    ...
    "person8":[
        ...
        {"text2":"This is Univsersity Edge. The following violations have been reported."}
    ],
    "person9":[
        {"photo":"https://cdn-icons-png.flaticon.com/128/17878/17878705.png"},
        {"head":"Jason Tatum"},
...
for (let key in data){
    if (data.hasOwnProperty(key)) {
        let person = data[key];
        text += '<div class="grid">';
        text += '<img class="petco" src="' + person[0].photo + '" class="image">';
        text += '<div class="info">';
        text += '<div class="head" id="head">' + (person[1].head || '') + '</div>';
        text += '<div class="email email1" id="pad1">' + (person[2].text1 || '') + '</div>';
        text += '<div class="email email2" id="pad2">' + (person[3].text2 || '') + '</div>';
        text += '</div></div>';
    }  
} 
...
function searchContent() {
    ...
    divs.forEach(div => {
        ...
        head.innerHTML = head.innerHTML.replace(/<span class="highlight">|<\/span>/g, '');
        email1.innerHTML = email1.innerHTML.replace(/<span class="highlight">|<\/span>/g, '');
        email2.innerHTML = email2.innerHTML.replace(/<span class="highlight">|<\/span>/g, '');
        ...
        if (searchQuery) {
            ...
            if (head && head.innerHTML.match(sens)) {
                head.innerHTML = head.innerHTML.replace(sens, match => `<span class="highlight">${match}</span>`);
                foundMatch = true;
            }
            if (email1 && email1.innerHTML.match(sens)) {
                email1.innerHTML = email1.innerHTML.replace(sens, match => `<span class="highlight">${match}</span>`);
                foundMatch = true;
            }
            if (email2 && email2.innerHTML.match(sens)) {
                email2.innerHTML = email2.innerHTML.replace(sens, match => `<span class="highlight">${match}</span>`);
                foundMatch = true;
            }
            if (foundMatch) {
                div.scrollIntoView({
